...
import { Download, Upload, FolderPlus, Trash2, X } from "lucide-react";
...
import { getDepartmentColumns } from "./columns/department";
...
export function DepartmentsPage() {
  const {
    ...
    confirmDelete,
    refetch,
  } = useResourceTable({ api: departmentsApi, pageSize: 8 });
  ...
  async function handleDownloadTemplate() {
    try {
      await downloadFile(
        "/departments/csv-template",
        "department-upload-template.csv",
      );
    } catch (error) {
      setLoadError(
  ...
  const columns = getDepartmentColumns(
    (id) => setEditTarget(departments.find((s) => s.id === id) ?? null),
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage departments for the mobilization portal.
          </p>
        </div>
      ...
      <div className="rounded-md border">
        <DataTable
          ...
          getRowId={(row) => row.id}
          emptyMessage="No departments found. Try a different search, or add one."
        />
        <DataTablePagination
          page={page}
      ...
      <CsvUploadDialog
        ...
        uploadFn={departmentsApi.bulkUpload}
        title="Bulk upload departments"
        description="Upload a CSV file using the template format to add multiple departments at once."
      />
